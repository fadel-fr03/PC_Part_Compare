...
export function CompareProvider({ children }) {
  ...
  function toggle(part) {
    ...
    setItems((prev) => {
      ...
      if (prev.length >= 3) {
        showToast("Max 3 parts only.");
        return prev;
      }

      if (prev.length > 0 && prev[0].category !== part.category) {
        showToast("Compare parts from the same category only.");
        return prev;
      }
    ...
    <CompareContext.Provider value={value}>
      ...
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[999]">
          <div className="rounded-xl px-4 py-2 text-sm font-medium bg-slate-900/90 backdrop-blur ring-1 ring-white/10 text-white shadow-lg">
            {toast}
          </div>
        </div>
      )}
